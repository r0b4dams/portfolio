...
const useDarkMode = () => {
  const [darkModeActive, set] = useState<boolean>(false);
  ...
  const toggleDarkMode = () => {
    set((mode) => {
      if (ref.current) {
        ...
        return ref.current.classList.toggle('dark', !mode);
      }
      return false;
    });
  };

  return { darkModeActive, toggleDarkMode };
};

export const ToggleDarkMode: React.FC<IProps> = () => {
  const { darkModeActive, toggleDarkMode } = useDarkMode();

  return (
    <button className='md:mx-5 md:mb-2' onClick={toggleDarkMode}>
      {darkModeActive ? (
        <svg
